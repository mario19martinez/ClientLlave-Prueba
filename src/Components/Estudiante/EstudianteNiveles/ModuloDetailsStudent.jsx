...
function ModuloDetailsStudent() {
  ...
  useEffect(() => {
    const fetchRespuestasUsuario = async () => {
      if (userSub) {
        try {
          const response = await axios.get(`/resultados/${userSub}/${moduloId}`);
          if (response.data.length > 0) {
            const respuestasData = response.data[0].preguntas;
            setRespuestasUsuario(respuestasData);
            const formattedRespuestas = respuestasData.reduce((acc, current, index) => {
  ...
  const todasCorrectas = () => {
    return modulo && modulo.preguntas.every((pregunta, index) => {
  ...
  return (
    <div className="mx-auto max-w-4xl py-10 px-4 sm:px-6 lg:px-8">
      ...
      {mostrarPreguntas && (
        <div>
          <h3 className="text-2xl font-bold text-gray-800 mb-4">Preguntas:</h3>
              ...
              {modulo.preguntas.map((pregunta, index) => (
                <div key={index} className="mb-6">
                  ...
                  {quizCompletado && respuestas[index] && (
                    <div className={`mt-2 ${verificarRespuesta(index, respuestas[index]) ? "text-green-600" : "text-red-600"}`}>
                      {verificarRespuesta(index, respuestas[index]) ? "Correcto" : "Incorrecto. Sigue intentado, no te rindas"}
                    </div>
                  )}
                </div>
              ))}
