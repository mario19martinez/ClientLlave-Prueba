...
export default function DocumentoModulo({ certificadoId, sub }) {
    ...
    const fetchCertificado = async () => {
      try {
        ...
        if (response.status === 404) {
          alert("Certificado de módulo no encontrado");
        } else {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.put(`/certificadosModulo/${certificadoId}`, {
        tipoDocumento,
        documento,
      });
