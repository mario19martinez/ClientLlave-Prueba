...
export default function AgregarDocumentos({ idCertificado, idUser }) {
  const [tipoDocumento, setTipoDocumento] = useState("");
  const [documento, setDocumento] = useState("");
  const [userInfo, setUserInfo] = useState(null);
  const [tiposDocumentos, setTiposDocumentos] = useState([]);

  console.log('este es el id del certificado desde el modal: ', idCertificado)
  ...
  useEffect(() => {
    const fetchUserInfo = async () => {
      try {
        ...
        const paisInfo = documentos.paises.find(pais => pais.nombre === paisUsuario);
  ...
  useEffect(() => {
    const fetchCertificado = async () => {
      try {
        const response = await axios.get(`/certificadosCurso/${idCertificado}`);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.put(`/certificadosCurso/${idCertificado}`, {
        tipoDocumento,
        documento,
      });
