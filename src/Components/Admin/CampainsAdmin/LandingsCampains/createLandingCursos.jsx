...
export default function CreateLandingCursos({ campeinId, templateProp }) {
  ...
  console.log('Este es el id de la campaña: ',campeinId)
  ...
  useEffect(() => {
    const fetchCursos = async () => {
      try {
        const response = await axios.get("/cursos-gratuitos");
        setCursos(response.data);
      } catch (error) {
        console.error("Error al obtener los cursos gratuitos:", error);
      }
    };
  ...
  console.log("datos cursos: ", selectedCurso)
  ...
  return (
    <div className="w-full px-20 py-8">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          <label htmlFor="template" className="block font-semibold">
            Template:
          </label>
          <input
            type="text"
